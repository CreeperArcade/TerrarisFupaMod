...
using System;
using TestModSword.Items.Sword;

namespace TestModSword.NPCs.FUPA_boss
{
    [AutoloadBossHead]

    public class FUPAboss : ModNPC
    {
        ...
        public float divideForSecondStageAnimations;
        ...
        {
            DisplayName.SetDefault("FUPA-UL LUI GETA");
        ...
        {
            if(item.type != ModContent.ItemType<DickSword>())
            {
        ...
        {
            if (projectile.type != ModContent.ItemType<DickSword>())
            {
                NPC.life = NPC.lifeMax;
            }
        }
        ...
        public override void AI()
        {
            ...
            if (NPC.ai[0] == 0f)
            {
                if (NPC.ai[1] == 0f)
                {
                    ...
                    else if ((NPC.position.Y + (float)NPC.height < Main.player[NPC.target].position.Y && num557 < 500f) || (Main.expertMode && num557 < 500f))
                    {
                        ...
                        if (NPC.ai[3] >= num670)
                        {
                            ...
                            {
                                int num727 = NPC.NewNPC(GetSpawnSourceForNPCFromNPCAI(), (int)position.X, (int)position.Y, 5);
                ...
                if ((float)NPC.life < (float)NPC.lifeMax * num793) //second stage go //
                {
                    NPC.ai[0] = 1f;
                    NPC.ai[1] = 0f;
                    NPC.ai[2] = 0f;
                ...
                divideForSecondStageAnimations = num793;
                return;
            }
            if (NPC.ai[0] == 1f || NPC.ai[0] == 2f)
            {
                ...
                if (Main.expertMode && NPC.ai[1] % 20f == 0f)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        int num849 = NPC.NewNPC(GetSpawnSourceForNPCFromNPCAI(), (int)position3.X, (int)position3.Y, 5);
            ...
            {
                SoundEngine.PlaySound(SoundID.ForceRoar, position);
                NPC.rotation = num1259;
                float num960 = 6.8f;
        ...
        {
            npcLoot.Add(ItemDropRule.Common(ItemID.RedPotion, 1, 1, 5));
        }
    }
}
